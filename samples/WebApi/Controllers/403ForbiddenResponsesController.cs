...
{
	/// <summary>
	///		Converts <see cref="IDomainResult"/>, <see cref="IDomainResult{T}"/> and `(T, <see cref="IDomainResult"/>)` responses with <see cref="IDomainResult.Status"/>='Error' to <see cref="ForbidResult"/>
	/// </summary>
	[ApiController]
	[Route("[controller]")]
	public class ForbiddenResponsesController : ControllerBase
	{
		private readonly DomainUnauthorizedService _service = new DomainUnauthorizedService();
