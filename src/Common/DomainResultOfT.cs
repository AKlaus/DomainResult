...
namespace DomainResults.Common
{
	...
	public class DomainResult<TValue> : IDomainResult<TValue>
	{
		...
		public static IDomainResult<TValue> NotFound(IEnumerable<string> messages) =>
			new DomainResult<TValue>(DomainResult.NotFound(messages));

		/// <summary>
		///		Returns <see cref="DomainOperationStatus.Unauthorized"/> status. Gets converted to HTTP code 403 (Forbidden)
		/// </summary>
		/// <param name="message"> Optional message </param>
		public static IDomainResult<TValue> Unauthorized(string? message = null) =>
			new DomainResult<TValue>(DomainResult.Unauthorized(message));
		...
		public static IDomainResult<TValue> Failed(IEnumerable<ValidationResult> validationResults) =>
			new DomainResult<TValue>(DomainResult.Failed(validationResults));
		...
		public static IDomainResult<TValue> CriticalDependencyError(string? error = null) =>
			new DomainResult<TValue>(DomainResult.CriticalDependencyError(error));
		...
		{
			if (result.IsSuccess) {
				throw new InvalidOperationException("Cannot implicitly convert a successful result.");
			}

			return From(result) as DomainResult<TValue> ?? throw new InvalidOperationException();
		}
	}
}
